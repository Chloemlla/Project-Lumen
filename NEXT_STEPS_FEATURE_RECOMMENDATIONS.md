...
- 统计：支持用眼统计、番茄统计、近 7 天展示、CSV 分享、统计图片分享。
- 设置：支持语言、主题、提醒间隔、休息时长、预提醒、声音、自定义音频路径、通知开关、自动更新检查。
- 模板：支持内置模板、系统主题色模板、自定义图片路径。
- 通知：支持通知渠道、精确闹钟能力检测、Android 13+ 通知权限引导、前台服务常驻状态通知。
- 更新：支持 GitHub Release 最新版本检测、语义版本与发布时间双判定、ABI APK 选择、下载进度、未知来源安装授权跳转。
- 崩溃报告：本地保存崩溃信息，启动后展示崩溃详情页。
- 国际化：已有中文、英文和系统语言逻辑。

### 1.2 已有技术结构

- `ProjectLumenViewModel` 当前承担主要状态机、设置写入、统计聚合、通知刷新和导出入口。
- `AppDatabase` 使用 Room，当前版本为 3，包含设置、运行态、用眼统计、番茄统计、模板表。
