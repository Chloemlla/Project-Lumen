...
- 后续（2026-09-13）：曾按 ① 接入，但随即被用户报为缺陷——它与 `SettingsPrivacyPermissionCenter`（8823b0b 引入）**同标题**（都取 `R.string.eye_care_privacy_permissions`）且相邻，设置页因此连续出现两个「隐私与权限」，其中旧卡还是不可折叠的普通 `Card`。该卡内容（配置分 / 7 条权限说明 / `eye_care_privacy_boundary`）是交互版的真子集，连 `detailRes` 都复用同一批字符串，两个百分比还取自不同公式（`configurationScore` vs `readinessScore`）。故改按 ② 删除本卡的 UI 部分（`PermissionTransparencyLine` 一并删除），权限透明度信息由交互式隐私中心承载。`SystemBackgroundPicker` 仍未处置。
