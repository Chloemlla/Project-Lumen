...
## Aura integration reference

[Aura](https://github.com/Chloemlla/Aura) was the first external Android app to integrate `lumen-crash` as a published SDK (not monorepo module). This section documents that integration as a reference for future adopters.

### Repository structure

```
Aura/
├── lumen-crash.version          # version policy file: "latest" sentinel
├── lumen-crash.resolved.version # generated by CI fetch script (gitignored)
├── local-maven/                 # materialized Maven artifacts (gitignored)
├── .github/
│   ├── scripts/
│   │   └── fetch-lumen-crash-sdk.py   # version resolution + artifact materialization
│   └── workflows/
│       └── aura-android.yml           # CI pipeline with fetch + build
├── settings.gradle.kts                # GitHub Packages + local-maven repos
├── app/
│   ├── build.gradle.kts               # version resolution + dependency
│   ├── proguard-rules.pro             # keep + dontwarn for lumen-crash
│   └── src/main/java/com/chloemlla/aura/
│       ├── AuraApp.kt                 # installLumenCrashSdk() in attachBaseContext + onCreate
│       ├── MainActivity.kt            # LumenCrashGate wrapper
│       └── service/
│           └── CrashDiagnosticsCollector.kt  # adapted to LumenCrash.loadPendingReportSafely()
└── gradle/
    └── verification-metadata.xml      # trusted-artifacts for lumen-crash + lumen-crash-core
```

### Integration touchpoints

#### 1. Maven repository setup (`settings.gradle.kts`)

Two repositories for the SDK: GitHub Packages (primary, authenticated) and local-maven (fallback, materialized by CI fetch script):
...
#### 2. Version resolution (`app/build.gradle.kts`)

Three-tier resolution: resolved version file (from CI fetch) > version file > explicit error:
...
val lumenCrashSdkVersion: String =
    readVersionFile(rootProject.file("lumen-crash.resolved.version"))
        ...
        ?: error("lumen-crash SDK version is unresolved...")
...
The `lumen-crash.version` file contains just `latest` (the sentinel that triggers auto-resolution from GitHub Releases). The `lumen-crash.resolved.version` file is generated by the CI fetch script and contains the concrete version like `0.1.0-fe5d9847`.

#### 3. SDK install (`AuraApp.kt`)

Called in **both** `attachBaseContext` and `onCreate` — idempotent via `LumenCrash.isInstalled()`:
...
The dual-call pattern ensures the SDK is installed as early as possible (in `attachBaseContext`, before any DI or other bootstrap) while also covering the `onCreate` path for robustness. The `runCatching` wrapper prevents install failures from crashing app startup.

#### 4. Pending-report UI gate (`MainActivity.kt`)

`LumenCrashGate` wraps the entire app content:
...
#### 5. Crash diagnostics adaptation (`CrashDiagnosticsCollector.kt`)

The existing diagnostics collector was adapted to read from lumen-crash instead of the old `crash.log` file:
...
`sanitizedCrashLogTail()` also uses `LumenCrash.loadPendingReportSafely()` to extract the stack trace, exception type, and root cause for the diagnostics bundle.

#### 6. ProGuard rules (`app/proguard-rules.pro`)
...
#### 7. Host product strings (`res/values/strings.xml`)
...
#### Fetch script (`fetch-lumen-crash-sdk.py`)

A Python script that runs at the start of every CI job. It:

1. Reads the version policy from `lumen-crash.version` (default `latest`)
2. Queries the GitHub Releases API for `lumen-crash-v*` tagged releases, filtering by tag prefix and sorting by publication date
3. Resolves the newest non-draft release (preferring main auto-releases in `X.Y.Z-<shortSha>` format)
4. Writes the resolved version to `lumen-crash.resolved.version`
5. Downloads the `.aar`, `.pom`, and optional `.module` / `checksums.txt` / `sdk-manifest.json` assets
6. Materializes them into the `local-maven/` directory with standard Maven coordinates layout
7. Optionally updates `gradle/verification-metadata.xml` with SHA256 checksums
...
The script supports:
- `--version` for explicit version pinning
- `--prefer-any-lumen-crash-tag` to accept pure semver tags (not just main auto-releases)
- `LUMEN_CRASH_SDK_VERSION` env var override
- `--verification-metadata` to control the metadata file path

#### Workflow credential setup

Cross-repo GitHub Packages access requires a PAT with `read:packages`. Aura stores this as `LUMEN_CRASH_READ_PACKAGES_TOKEN`:
...
All Gradle build steps pass the same token:
...
The workflow has `packages: read` permission at the job level:
...
|---|---|---|
| `401 Unauthorized` for `lumen-crash-core` | Cross-repo `GITHUB_TOKEN` cannot access packages from another repo | Use a PAT with `read:packages`, stored as `LUMEN_CRASH_READ_PACKAGES_TOKEN` secret |
| Gradle dependency verification failure | 38 artifacts (lumen-crash, lumen-crash-core, and transitive deps) missing SHA256 checksums | Add versionless `trusted-artifacts` entries for `lumen-crash` and `lumen-crash-core`; add `-Ddependency.verification=off` to CI Gradle commands; **also add `dependency.verification=off` to `gradle.properties`** (Gradle 9.6.1 does not reliably read the system-property form, so `gradle.properties` is the definitive fix); update fetch script to auto-update verification metadata |
| `rootProject.file("../lumen-crash.resolved.version")` path bug | `../` goes above repo root — CLens has `android/` subdirectory, Aura does not | Changed to `rootProject.file("lumen-crash.resolved.version")` (repo root) |
| `import javax.inject.Inject` accidentally removed | Import replacement during migration dropped the `@Inject` annotation import | Added `import javax.inject.Inject` back |
| Dependency verification metadata goes stale on version update | `lumen-crash` version changes on every `main` push (auto-releases use `<sdk.version>-<shortSha>`) | Fetch script dynamically updates `verification-metadata.xml` with SHA256 checksums of downloaded artifacts; CI uses `-Ddependency.verification=off` as safety net |
| `AutoboxingStateCreation` lint crash | `IncompatibleClassChangeError` in AGP 8.7.3 / Kotlin 2.1.0 analysis-api — the `AutoboxingStateCreationDetector` crashes during `lintAnalyzeFullDebug` | Add `disable += "AutoboxingStateCreation"` to the `lint` block in `app/build.gradle.kts`. Same class of analysis-api incompatibility as the already-disabled `NullSafeMutableLiveData`, `FrequentlyChangingValue`, and `RememberInComposition` detectors |
| Package name migration in test assertions | `appPrivatePathRegex` in `CrashDiagnosticsCollector` and `TaskerActionReceiverContractTest` README assertions still referenced `com.freevibe` (old package name) instead of `com.chloemlla.aura` | Updated regex to `com\.chloemlla\.aura` and README action constants to `com.chloemlla.aura.action.*` |

### Key design decisions

1. **Versionless trusted-artifacts**: `lumen-crash` and `lumen-crash-core` are trusted via `<trust group="com.chloemlla.lumen" name="lumen-crash"/>` (no version constraint) so the verification metadata does not need manual updates on every version change.

2. **Dual install call**: `installLumenCrashSdk()` in both `attachBaseContext` and `onCreate` — the `isInstalled()` guard makes this idempotent. This ensures the SDK is active before any app bootstrap code while also covering the standard `onCreate` lifecycle.

3. **Local-maven as fallback**: Materializing artifacts into `local-maven/` via the fetch script means the build can resolve the SDK even if GitHub Packages is temporarily unavailable. The local-maven repo is listed before GitHub Packages in `settings.gradle.kts`.

4. **`runCatching` on install**: All lumen-crash calls in the app are wrapped in `runCatching` / `runCatching { ... }.getOrNull()` to prevent SDK failures from crashing the host app. This follows the fail-soft host pattern recommended in the integration pitfalls section.

5. **`CrashDiagnosticsCollector` adaptation**: Rather than replacing the diagnostics system entirely, the collector was adapted to use `LumenCrash.loadPendingReportSafely()` as its crash data source, preserving the existing diagnostics bundle format and user-facing features.

6. **`SHORT_HASH` buildConfigField**: The app's `build.gradle.kts` adds a `BuildConfig.SHORT_HASH` field sourced from `local.properties` or `AURA_SHORT_HASH` env var (fallback `"unknown"`). This is passed to `LumenCrashConfig.commitHash` so crash reports include the exact build commit for debugging — without exposing the full hash or requiring a Git plugin at configure time.

### CI pipeline order

Each CI job (build, verify, release) follows this order:

1. Checkout repository
2. Set up Java 21
3. Set up Gradle with caching
4. **Resolve lumen-crash SDK version** (fetch script)
5. Read app version
6. Validate/build secrets
7. Stop stale Gradle daemons (ensures workers pick up the correct `JAVA_HOME`)
8. Gradle build with `-Pdependency.verification=off` (also set `dependency.verification=off` in `gradle.properties` as the definitive fix for Gradle 9.6.1+)

This ensures the SDK version is resolved and materialized before Gradle configuration, so `lumen-crash.resolved.version` exists when `app/build.gradle.kts` reads it.

### AGP 8.9.1 + compileSdk 36 upgrade

After the initial lumen-crash integration, Aura upgraded its Android Gradle Plugin from 8.7.3 to 8.9.1 and compileSdk/targetSdk from 35 to 36. This was driven by transitive dependency requirements (`activity-1.13.0` needs AGP 8.9.1+, `activity-compose-1.13.0` needs compileSdk 36). The upgrade triggered a cascade of CI failures — documented here as a reference for any host app that bumps these versions.
...
| `gradle/verification-metadata.xml` | **Deleted** (11077 lines, see below) |
| `.github/workflows/aura-android.yml` | All 8 `gradle` → `./gradlew`; `java-version: "17"` → `"21"`; `-Ddependency.verification=off` → `-Pdependency.verification=off`; added "Stop stale Gradle daemons" step; test commands prefixed with `cleanTest` and `-Dorg.gradle.java.home="$JAVA_HOME"`; release script handles ABI-split APK naming |
| `gradlew` | `git update-index --chmod=+x gradlew` (mode 100644 → 100755) |
| `app/.../AudioPlaybackService.kt` | `LibraryResult.RESULT_ERROR_BAD_VALUE` → `SessionError.ERROR_BAD_VALUE` (2 calls); added `import androidx.media3.session.SessionError` |

#### CI failure chain and fixes

| CI run | Symptom | Root cause | Fix |
|---|---|---|---|
| #31489496213 | AGP plugin verification failure | `com.android.application.gradle.plugin-8.9.1.pom` not in dependency verification metadata | Added versionless `trusted-artifacts` entry |
| #31489662084 | 120 AGP build tools artifacts verification failure | All 8.9.1 build tools classpath artifacts not in verification metadata | **Deleted `verification-metadata.xml`** — the 11077-line file was impossible to maintain; `gradle.properties` already has `dependency.verification=off` |
| #31489875044 | `Permission denied` (exit code 126) | `./gradlew` lacks executable permission in git | `git update-index --chmod=+x gradlew` |
| #31490436647 | Robolectric sandbox: "Android SDK 36 requires Java 21 (have Java 17)" | `setup-java` set `JAVA_HOME` to 21, but system Java was 17; Robolectric 4.16.1's `DefaultSdkProvider.verifySupportedSdk()` checks the runtime Java version | CI Java version 17 → 21 |
| #31492082715 | Robolectric still fails — Gradle workers use Java 17 | Gradle daemon workers inherit the system Java, not `JAVA_HOME` | Added `-Dorg.gradle.java.home="$JAVA_HOME"` to test commands; added "Stop stale Gradle daemons" step; prefixed tests with `cleanTest` to remove stale Robolectric sandbox |
| #31493485275 | `lintFullDebug`: `WrongConstant` — `LibraryResult.ofError` | `LibraryResult.ofError()` is annotated with `@IntDef(SessionError.ERROR_*)` and rejects `LibraryResult.RESULT_*` constants | `RESULT_ERROR_BAD_VALUE` → `SessionError.ERROR_BAD_VALUE` |
| #31495924951 | Compilation: "Unresolved reference 'SessionError'" | `SessionError` class was not imported | Added `import androidx.media3.session.SessionError` |
| #31496207070 | Release script: no universal APK found | ABI splits enabled (`isUniversalApk = false`), so `app-*-universal-release.apk` does not exist | Release script uses `find -o -name "app-${flavor}-arm64-v8a-release.apk"` as fallback |

#### Key lessons

1. **`verification-metadata.xml` is not worth the maintenance cost** when the team already accepts `dependency.verification=off` in `gradle.properties`. Deleting the 11K-line file was the fastest fix.

2. **`-D` vs `-P` for Gradle properties**: `dependency.verification` is a Gradle property (read from `gradle.properties`), not a system property. CI must use `-Pdependency.verification=off` (project property), not `-Ddependency.verification=off` (system property). The `gradle.properties` file is the definitive fix because Gradle daemons spawned from CI runners may not pass `-D` flags to workers.

3. **`gradle` vs `./gradlew`**: The system `gradle` command may provide a different Gradle version than the wrapper. Switching to `./gradlew` ensures reproducible builds. On CI runners, `./gradlew` also needs `chmod +x` tracked in git.

4. **Java 21 for Robolectric + SDK 36**: Robolectric 4.16.1's `DefaultSdkProvider.verifySupportedSdk()` checks the **runtime** Java version, not `JAVA_HOME`. Even with `setup-java` setting Java 21, the Gradle daemon and its workers may still run Java 17. The fix is `-Dorg.gradle.java.home="$JAVA_HOME"` which forces Gradle workers to use the correct JDK. Running `cleanTest<variant>` before tests removes stale Robolectric sandbox state from previous Java versions.

5. **`LibraryResult.ofError()` accepts `SessionError` constants, not `RESULT_*`**: The `@IntDef` annotation on `LibraryResult.ofError()` restricts parameters to `SessionError.ERROR_*` values. Using `LibraryResult.RESULT_ERROR_BAD_VALUE` (which is a `@RESULT_*` constant) triggers a `WrongConstant` lint error. The fix is `LibraryResult.ofError(SessionError.ERROR_BAD_VALUE)` with the explicit `SessionError` import.

6. **ABI split APK naming**: When `splits.abi.isEnable = true` and `isUniversalApk = false`, there is no `app-*-universal-release.apk`. Release scripts that previously expected a universal APK need to search for the per-ABI variants instead. Aura targets arm64-v8a as the primary APK (~95% of modern Android), and copies all other ABI variants separately.

7. **`runCatching` silently swallows `LumenCrash.install()` exceptions**: The `installLumenCrashSdk()` method wraps `LumenCrash.install()` in `runCatching {}` without logging. If the SDK fails to install (e.g., due to a missing dependency or integrity check), the failure is completely invisible. Consider adding `.onFailure { Log.w(...) }` to the `runCatching` block for debugging.
